...
// DB 부트스트랩(require 실패 영구 캐시 / 인스턴스 생성 실패 일시적 구분 / isNew chmod 0600 등)은
// graphLayer.js 로 공유 추출됨(void-context 스택도 동일 패턴을 쓰기 때문) — 이 파일은 이제
// dbFile/namespace/시드 콜백만 넘기고 getGraph/ensureSeeded/dbPath 를 그대로 재노출한다.
const { initVoidGraphLayer } = require('./graphLayer');
const { getGraph, ensureSeeded, dbPath } = initVoidGraphLayer({
  dbFile: path.join(storageDir(), 'aggregator.djinn.db'),
  namespace: NS,
  nodeDefs: NODE_DEFS,
  seed: seedAggregatorDocs,
});
...
// dJinn 불가 시 이 함수 자체가 호출되지 않는다(graphLayer.js 의 ensureSeeded() 가 getGraph()
// null 이면 seed 콜백을 부르지 않고 바로 리턴 — 모든 읽기 경로는 DEFAULT_* 상수로 폴백).
function seedAggregatorDocs(g) {
