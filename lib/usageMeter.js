...
function okResult(session, weekly, source, fetchedAt) {
  return { status: 'ok', session, weekly, error: null, source, fetchedAt: fetchedAt || null };
}
...
async function fetchClaudeUsageResult(_config, overrideDir, sessionKey) {
  // Tier -1, highest priority: omniroute already holds session/weekly windows for
  // every connected account (it polls them itself) — if the active session's
  // email matches one, use it directly and skip network/PTY entirely. Wrapped in
  // try/catch and any miss (disabled, no router, no match) falls through unchanged.
  try {
    const { deriveClaudeEmail, lookupOmnirouteUsage } = require('./void-persistent/omnirouteUsageTier');
    const email = deriveClaudeEmail(claudeConfigDir(overrideDir));
    const found = email && lookupOmnirouteUsage('claude', { email });
    if (found) return okResult(found.session, found.weekly, 'omniroute', found.fetchedAt);
  } catch {
  ...
  try {
    const { checkLocalRateLimit } = require('./void-persistent/localLogTier');
...
async function fetchCodexUsageResult(_config, overrideDir, sessionKey) {
  // Tier -1, highest priority — same rationale as the Claude chain above.
  try {
    const { deriveCodexAccountId, lookupOmnirouteUsage } = require('./void-persistent/omnirouteUsageTier');
    const accountId = deriveCodexAccountId(codexHomeDir(overrideDir));
    const found = lookupOmnirouteUsage('codex', { accountId });
    if (found) return okResult(found.session, found.weekly, 'omniroute', found.fetchedAt);
  } catch {
    // fall through to the existing chain
  }
...
// ── agy (Antigravity CLI) ────────────────────────────────────────
//
// agy has no `/usage` or `/status` of its own — its Gemini per-model quotas
// only exist inside omniroute (see void-persistent/omnirouteUsageTier.js).
// There is deliberately no local/PTY fallback chain here: if omniroute has no
// matching connection, agy usage is simply unavailable.
function agyConfigDir(overrideDir) {
  return overrideDir || process.env.AGY_CONFIG_DIR || process.env.AGY_HOME || null;
}

async function fetchAgyUsageResult(_config, overrideDir, _sessionKey) {
  try {
    const { deriveAgyEmail, lookupOmnirouteUsage } = require('./void-persistent/omnirouteUsageTier');
    const email = deriveAgyEmail(agyConfigDir(overrideDir));
    const found = lookupOmnirouteUsage('agy', { email });
    if (found) return okResult(found.session, found.weekly, 'omniroute', found.fetchedAt);
  } catch {
    // fall through to unavailable below
  }
  return unavailableResult('agy 사용량은 omniroute 연결이 있어야 조회됩니다.', 'omniroute');
}
...
function resolveSessionKey(toolCommand) {
  const envVar = toolCommand === 'claude' ? 'CLAUDE_CONFIG_DIR'
    ...
    : toolCommand === 'agy' ? 'AGY_CONFIG_DIR'
    : null;
...
function withUsageCache(provider, toolCommand, fetchFn) {
  return async function cachedGetUsage(config, overrides) {
    ...
    if (result.status === 'ok') {
      ...
      // transient failure never clobbers the last-known-good value. When the
      // source is omniroute, result.fetchedAt carries omniroute's own
      // server-side fetch time — use that as the authoritative "last fetched"
      // timestamp (both in the cache row and the returned cachedAt) instead of
      // the local Date.now() at the moment void happened to call the API.
      // Every non-omniroute source never sets result.fetchedAt, so this falls
      // through to Date.now() exactly as before.
      const fetchedAtMs = typeof result.fetchedAt === 'number' && Number.isFinite(result.fetchedAt)
        ? result.fetchedAt
        : null;
      saveUsageCacheEntry(provider, sessionKey, result, fetchedAtMs);
      return { ...result, cachedAt: fetchedAtMs || Date.now(), stale: false };
    }
...
const getAgyUsage = withUsageCache('agy', 'agy', fetchAgyUsageResult);

module.exports = { getClaudeUsage, getCodexUsage, getAgyUsage, makeWindow };
