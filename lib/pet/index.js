...
// ── 감정 도출 ────────────────────────────────────────────────
// agentEmotion(에이전트가 embodiment MCP의 set_expression 으로 직접 지정한 표정)
// 최우선, 그다음 event-state, 그다음 vitals 임계값. moodState 는 setMood()/
// setState() 가 다루는 8-mood 이름(§MOOD8_TO_EMOTION16) 이다.
function deriveEmotion({ moodState, vitals, agentEmotion, now } = {}) {
  // 에이전트가 직접 지정한 표정 — 만료되지 않았고 알려진 16 어휘에 속하면
  // 기존 mechanical 분기(moodState/vitals)를 전부 건너뛰고 최우선 적용한다.
  // 이전에는 이 신호 자체가 없었다(주석 참고) — 여기서 처음 추가됨.
  if (agentEmotion && EMOTIONS.includes(agentEmotion.emotion)) {
    const nowTs = resolveNow(now);
    if (typeof agentEmotion.expiresAt !== 'number' || agentEmotion.expiresAt > nowTs) {
      return agentEmotion.emotion;
    }
  }
 ...
 *   그릴 수 있는 감정으로 접는다.
 * @property {(args: {emotion: string, vitals: object, animPhase?: {blink?: boolean, breathe?: 0|1}}) => string[]} drawSprite -
 ...
 *
 *   **animPhase** (선택, 기본 `{blink:false, breathe:0}`): lib/ui.js 의 idle
 *   애니메이션 드라이버(assistantChatView)가 매 틱 계산해 넘기는 순간 상태 —
 *   `blink:true` 인 틱 한 번만 눈을 감고, `breathe` 는 0/1 을 느린 주기로
 *   토글해 미세한 "숨쉬기"를 표현하는 데 쓴다. **인터페이스는 무엇을(두
 *   플래그) 넘길지만 정의하고, 그걸 자기 실루엣의 어느 셀로 어떻게 표현할지는
 *   전적으로 각 스킨이 정한다** — 예를 들어 space-invader 는 눈 문자를 '-' 로,
 *   가슴 칸 2개를 █→▓ 로 바꾸지만, 다른 스킨은 완전히 다른 셀을 바꿔도 된다.
 *   animPhase 를 생략(또는 기본값)하면 오늘의 정적 스프라이트와 바이트 단위로
 *   동일한 출력을 반환해야 한다(하위 호환 계약 — 기존 호출부/테스트가 깨지지
 *   않게).
 *
 ...
 * @property {{enabled?: boolean, blinkMinMs?: number, blinkMaxMs?: number, breathePeriodMs?: number}} [idleAnim] -
 *   선택적 capability/cadence 힌트(메타데이터일 뿐 — 실제 애니메이션 판단
 *   로직은 항상 lib/ui.js 의 드라이버가 갖는다). 스킨이 이 필드를 생략하거나
 *   일부만 채우면 나머지는 DEFAULT_IDLE_ANIM 으로 채워진다(아래
 *   getIdleAnimConfig 참고). `enabled: false` 를 명시하면 "이 스킨은 idle
 *   애니메이션을 받지 않겠다"는 뜻이고, lib/ui.js 는 이 스킨엔 애니메이션
 *   틱을 계산/전달하지 않는다.
 */

// idle 애니메이션 기본 cadence — 스킨이 idleAnim 을 아예 안 주거나 일부만 줄
// 때 나머지를 채운다. blink 는 "이 범위 안에서 확률적으로 한 번" 식으로
// 근사되고(정확한 스케줄러가 아님), breathe 는 고정 주기로 0/1 을 토글한다.
const DEFAULT_IDLE_ANIM = {
  enabled: true,
  blinkMinMs: 4000,
  blinkMaxMs: 7000,
  breathePeriodMs: 1750,
};

// 스킨별 idleAnim 오버라이드 + DEFAULT_IDLE_ANIM 병합. 스킨이 idleAnim 을 아예
// 안 가지고 있어도(과거/미래 스킨) 항상 완전한 설정 객체를 반환한다 — lib/ui.js
// 는 이 함수를 통해서만 cadence 를 읽어, 특정 스킨(예: space-invader)의 내부를
// 몰라도 된다(다형성 — 스킨을 바꿔도 이 호출부는 그대로 재사용된다).
function getIdleAnimConfig(skin) {
  return { ...DEFAULT_IDLE_ANIM, ...(skin && skin.idleAnim ? skin.idleAnim : {}) };
}
