...
// 컨트롤 패널(Ctrl+\ 로 토글)이 열려 있을 때만 의미 있는 글자 핫키 → lib/pet 의
// INTERACTIONS kind. 패널 밖(일반 채팅 입력 중)에는 아무 효과가 없다 — onKey 가
// panelOpen 일 때 다른 어떤 분기보다 먼저 이 표로 키를 소비하므로 채팅
// 입력(f/p/r/t 로 시작하는 메시지 등)과 절대 충돌하지 않는다.
const PET_INTERACT_KEYS = { f: 'feed', p: 'play', r: 'rest', t: 'pet' };
...
// Ctrl+\ 로 토글되는 컨트롤 패널의 본문(makeBox 로 감싸질 body lines). 이 화면
// 전용 오버레이 — xtermFrame 의 컨트롤 패널과 동작만 비슷할 뿐 완전히 별개
// 코드다(이 화면은 프레임 wrapper 밖의 독립 raw-mode 화면이라 그쪽을 재사용할
// 수 없음). skin 이 없으면 펫 액션 항목(F/P/R/T, vitals)은 의미가 없으니 숨기고
// 토큰 변경/닫기만 보여준다.
function buildPetPanelBody(skin, vitals) {
  const petLines = skin
    ? [
        '',
        ' [F] 밥        [P] 놀기',
  ...
  const vitalsLines = skin && vitals ? ['', ...buildPetVitalsLines(vitals)] : [];
  return [...petLines, ...footerLines, ...vitalsLines];
}
...
function assistantChatView(model) {
  ...
  // raw-mode 화면이라 프레임 컨트롤 패널을 재사용할 수 없어, 펫 상호작용
  // 단축키(F/P/R/T)와 토큰 변경(K)을 여기 안에 모아 넣는다. 열려 있는 동안은
  // onKey 가 다른 어떤 분기보다 먼저 이 상태를 확인해 채팅 입력으로 새지 않게 한다.
  let panelOpen = false;

  if (!process.stdin.isTTY) {
    closed = true;
    resolveDone();
    return { appendDelta() {}, finalizeTurn() {}, appendSystem() {}, setState() {}, setMood() {}, setSessionMeta() {}, setTurnUsage() {}, setPetVitals() {}, done };
  }
  ...
  const draw = () => {
    ...
    let lines;
    if (compact) {
      // 좁은 터미널엔 아바타 칼럼이 아예 없으므로, 패널이 열려 있으면 채팅 대신
      // 패널을 그 자리에 통째로 보여준다(둘을 나란히 놓을 공간이 없음).
      lines = panelOpen
        ? makeBox(chatInner, buildPetPanelBody(skin, petVitals), { title: '컨트롤 패널', height: boxHeight })
        : chatBox;
    } else {
      ...
      // idle 애니메이션은 없다(사용자 결정 — 터미널 부하/깜빡임 방지) — 이벤트가
      // 생겨 draw() 가 다시 불릴 때만 이 스프라이트도 다시 그려지는 정적 렌더.
      const emotion = skin ? pet.deriveEmotion({ moodState: mood, vitals: petVitals }) : null;
      const spriteSection = skin
        ? [
            ...
            ...pet.padToGrid(skin.drawSprite({ emotion, vitals: petVitals, frame: 0 }))
      ...
      // 패널이 열려 있으면 아바타 영역(Agent 박스)을 컨트롤 패널로 통째로
      // 바꿔치기한다 — 채팅은 그대로 보여 대화 중에도 vitals/상호작용이 가능.
      const avatarBox = panelOpen
        ? makeBox(avatarInner, buildPetPanelBody(skin, petVitals), { title: '컨트롤 패널', height: boxHeight })
    ...
    lines.push(C.muted2 + '  ↑↓ 스크롤  Enter 전송  Ctrl+\\ 컨트롤 패널  ESC 뒤로  /model /effort /mcp 사용 가능' + scrollHint + C.RESET);
  ...
  const onKey = (str, key) => {
    ...
    // 채팅 입력('f'/'p'/'r'/'t'/'k' 로 시작하는 메시지 등)이나 스크롤/Esc(채팅
    // 종료)로 새지 않게 하기 위함. 그 외 키는 전부 무시(패널 밖으로 새지 않음).
    if (panelOpen) {
      ...
      if (skin && PET_INTERACT_KEYS[lower]) {
        const kind = PET_INTERACT_KEYS[lower];
        try {
          if (model.onPetInteract) {
            const updated = model.onPetInteract(kind);
            if (updated) petVitals = updated;
          }
        } catch {}
        // 액션 후에도 패널은 열린 채로 둔다 — 갱신된 vitals를 보며 연속으로
        // 상호작용할 수 있게(닫으려면 Esc/Ctrl+\ 를 따로 눌러야 함).
        draw();
        return;
      }
      return;
