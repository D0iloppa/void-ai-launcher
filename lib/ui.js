...
function renderHome(model, selectedIndex, optionIndices) {
  ...
  if (cols < minCols || rows < 24) {
    ...
    const menuBox = makeHomeMenuBox(
      ...
      '↑↓ 이동   ←→ 옵션 변경   Enter 실행   U 사용량   R 재시작   : command   0 종료'
    );
    const lines = [...menuBox];
...
function buildMenuLines(title, items, selectedIndex, optionIndices, innerWidth, opts = {}) {
  ...
  if (opts.enableRename) {
    helpStr += '  r 이름변경';
  }
  if (opts.enableExport) {
    helpStr += '  e 내보내기';
  }
...
async function menu(title, items, opts = {}) {
  ...
  return new Promise(resolve => {
    ...
    const onKey = (str, key) => {
      ...
      if ((qwertyStr === 'r' || qwertyStr === 'R') && opts.enableRename) {
        const item = items[sel];
        if (item && !item.disabled) {
          cleanup();
          resolve({ ...item, action: 'rename' });
          return;
        }
      }
      if ((qwertyStr === 'e' || qwertyStr === 'E') && opts.enableExport) {
        const item = items[sel];
        if (item && !item.disabled) {
          cleanup();
          resolve({ ...item, action: 'export' });
          return;
        }
      }
...
async function homeMenu(model) {
  ...
  return new Promise(resolve => {
    ...
    const onKey = (str, key) => {
      ...
      if (matchIdx >= 0) { done(matchIdx); return; }
      // 'U' — hidden global hotkey for the usage popup (footer-hinted only, no
      // dedicated menu row: keeps the home item list untouched). Only takes
      // effect when no configured item already claims 'u' as its key.
      if (lstr === 'u') { done(null, { result: { key: 'U' } }); }
      // 'R' — hidden global hotkey for the reboot popup (footer-hinted only, no
      // dedicated menu row: keeps the home item list untouched). Only takes
      // effect when no configured item already claims 'r' as its key.
      if (lstr === 'r') { done(null, { result: { key: 'R' } }); }
    };
...
// The exact inner content width message()'s box wraps its body to — exposed
// so a caller that wants to pre-wrap a long unbreakable line itself (e.g.
// extTokens.js showing a full "export FOO=..." string with no truncation)
// can do so with a width that matches, instead of relying on makeBox's
// per-line truncateCols (which would cut the line short with '…').
function messageInnerWidth() {
  const cols = process.stdout.columns || 120;
  return (cols - (FRAME.hpad * 2)) - 2;
}

// opts.actions (optional): [{ key, label, run, flash }] — extra single-key
// shortcuts shown in the footer alongside "Enter 계속" that run a callback
// and flash a brief confirmation WITHOUT dismissing the message, so the
// caller can still press Enter afterwards. Passing no opts.actions (every
// pre-existing call site) reproduces the original behavior byte-for-byte —
// same footer text, same resolve-on-Enter-or-Ctrl+C flow.
async function message(text, opts = {}) {
  ...
  const render = () => {
    ...
    const hint = actions.length
      ? actions.map(a => `${a.key} ${a.label}`).join('   ') + '   Enter 계속'
      : 'Enter 키를 눌러 계속...';
    boxedLines.push(C.muted2 + '  ' + hint + C.RESET);
  ...
  return new Promise(resolve => {
    ...
    const onKey = async (str, key) => {
      ...
      if (busy || !actions.length || !str) return;
      const lstr = (hangulToQwerty(str) || str).toLowerCase();
      const action = actions.find(a => a.key && a.key.toLowerCase() === lstr);
      if (!action) return;
      busy = true;
      try { await action.run(); } catch (_) { /* action failures never break the view */ }
      if (action.flash) {
        await flashMessage(action.flash, 700);
        render();
      }
      busy = false;
    };
...
// blockquotes are all explicitly out of scope) — just enough that
// `**bold**` and `` `code` `` render styled instead of leaking literal
// asterisks/backticks into the transcript, plus a trivial leading
// #/##/### header and -/* bullet. Fenced ``` code blocks are handled
// separately, one level up, by renderChatBody() below (block-aware, so it
// has to run BEFORE this per-line pass).
//
...
// ── Fenced ``` code blocks for assistant/user chat bubbles ───────────────
// One level above renderInlineMarkdown: that function only ever sees ONE
// line at a time, but a fenced block is inherently multi-line, so it has to
// be carved out of the raw bodyText BEFORE the per-line inline pass runs (a
// stray `**` or backtick inside a code line must NOT be touched by
// renderInlineSpans). A fence opens on a line that is (ignoring trailing
// whitespace) just ``` optionally followed by a bare language word, and
// closes on a line that is exactly ```. Everything in between is emitted
// verbatim — no inline markdown, indentation preserved — behind a dim
// left-gutter so it reads as visually distinct from prose.
const FENCE_OPEN_RE = /^```(\w*)\s*$/;

// Renders one already-fenced-out code line (plain text, no ANSI) as one or
// more gutter-prefixed output lines. Reuses wrapCols for the hard-wrap since
// it's already CJK-aware (colWidth) and ANSI-split-safe — this input just
// happens to carry no ANSI codes of its own, so it degenerates to a plain
// char-by-char wrap. Every returned line opens+closes its own SGR (gutter
// color, then RESET, then code color, then RESET) so a wrapped continuation
// line never depends on state left open by the previous one, and nothing
// bleeds past the block into whatever renders next (avatar panel, next
// message, etc).
function renderCodeLine(rawLine, width) {
  const w = Math.max(1, width);
  const gutter = '│ ';
  const gutterColor = C.DIM + C.muted2;
  const codeColor = C.DIM + C.info;
  const avail = Math.max(1, w - colWidth(gutter));
  const wrapped = wrapCols(rawLine, avail);
  const chunks = wrapped.length ? wrapped : [''];
  return chunks.map(chunk => gutterColor + gutter + C.RESET + codeColor + chunk + C.RESET);
}

// Subtle, skippable header for a fenced block that named a language — e.g.
// ```python. No footer/closing border: the gutter on every code line already
// marks where the block ends (first non-gutter line after it).
function renderCodeBlockHeader(lang, width) {
  const w = Math.max(1, width);
  const prefix = '┌─ ' + lang + ' ';
  const fill = repeatChar('─', Math.max(0, w - colWidth(prefix)));
  return truncateCols(C.DIM + C.muted2 + prefix + fill + C.RESET, w);
}

function renderCodeBlock(codeLines, lang, width) {
  const out = [];
  if (lang) out.push(renderCodeBlockHeader(lang, width));
  for (const raw of codeLines) out.push(...renderCodeLine(raw, width));
  return out;
}

// renderChatBody(bodyText, width): drop-in replacement for the previous
// wrapCols(renderInlineMarkdown(bodyText), width) call in buildChatLines.
// Splits bodyText into a run of non-code / code segments, sends non-code
// segments through the existing renderInlineMarkdown + wrapCols pipeline
// UNCHANGED, and renders code segments as gutter-prefixed code blocks. When
// bodyText contains no fence at all, this walks every line into a single
// non-code segment and reduces to exactly the old call — same output,
// byte-for-byte (verified in test/ui.test.js).
//
// Streaming: buildChatLines() re-runs this on the accumulating message text
// every draw(), including mid-stream. An opening fence with no closing ```
// yet simply has nothing to stop the scan — every remaining line is treated
// as code-in-progress and rendered as a (possibly still-growing) code block,
// so a code block appears the moment the ``` lands rather than only once the
// closing fence has streamed in. A lone unclosed ``` with nothing after it
// (or a named-but-empty block) degrades to just the optional header line
// (or nothing at all if there's no language) — least-ugly option, since
// there is no code content yet to show a gutter for.
function renderChatBody(bodyText, width) {
  const w = Math.max(1, width);
  ...
  const flushText = () => {
    ...
    out.push(...wrapCols(renderInlineMarkdown(textBuf.join('\n')), w));
    textBuf = [];
  };
  ...
  while (i < lines.length) {
    ...
    if (m) {
      ...
      out.push(...renderCodeBlock(codeLines, lang, w));
...
// ←→로 순환)와 Reasoning(체크박스, Space/Enter로 토글) 3행 + 펫 액션
// 힌트(F/P/R/T) + 푸터 힌트. 아바타 자체는 우측 사이드바에 그대로 남고, 여기
// 는 그 아바타를 조작하는 단축키 안내만 텍스트로 얹는다(사용자 결정: 714af26
// 에서 빠졌던 다마고치 컨트롤을 이 설정 패널로 복원). innerWidth 는
// overlayCenteredBox 가 계산하는 boxW-2 와 같은 값을 draw() 호출부에서 미리
// 계산해 넘긴다(포커스 행 배경색을 박스 폭 끝까지 정확히 칠하기 위해 필요).
function buildAssistantSettingsPanelBody(focus, modelIdx, effortIdx, showThinking, innerWidth, settingsPending) {
  ...
  return [
    ...
    // model/effort 변경 안내 — 대화창이 아니라 이 패널 안에서만 표시(사용자 결정).
    ...(settingsPending ? [C.warn + '⚠ 다음 메시지부터 적용' + C.RESET] : []),
    '',
    // 한 줄로 몰아넣으면(과거 버전) ~30컬럼 오버레이 폭에서 통째로 잘려
    // K(토큰변경)/L(이전대화) 힌트가 아예 안 보였다 — 여러 줄로 나눠 전부
    // 보이게 한다. overlayCenteredBox 가 bodyLines.length 로 박스 높이를
    // 다시 재는 구조라 줄이 늘어도 박스가 알아서 더 커질 뿐 잘리지 않는다.
    C.muted2 + '↑↓ 이동   ←→ 변경' + C.RESET,
    C.muted2 + 'Space 토글' + C.RESET,
    C.muted2 + 'K 토큰변경   L 이전대화' + C.RESET,
    '',
    // 펫 액션(원래 714af26 의 [F]밥 [P]놀기 [R]잠 [T]쓰담 — 설정 패널
    // 재설계 때 빠졌다가 여기로 복원). 두 줄로 나눠 ~28컬럼 안에 맞춘다.
    C.muted2 + 'F 밥   P 놀기' + C.RESET,
    C.muted2 + 'R 잠   T 쓰담' + C.RESET,
    C.muted2 + 'Esc 닫기' + C.RESET,
  ];
}
...
function assistantChatView(model) {
  ...
  let state = 'idle';
  let mood = 'idle';
  // 사고 과정(thinking) — 예전엔 트랜스크립트에 'think' 엔트리로 인라인
  // 렌더링됐지만, 우측 "Agent" 패널로 옮겼다(사용자 결정: 트랜스크립트는
  // You/응답/🔧 도구 카드만). 최신 thinking 블록 문자열만 들고 있다가 draw()
  // 의 아바타 패널이 state==='thinking' && showThinking 일 때만 그린다.
  // 턴이 끝나거나(finalizeTurn) idle로 돌아가면(setState) 비운다.
  let liveThinking = '';
  let scrollBack = 0;
  ...
  // 입력으로 새지 않게 한다.
  let panelOpen = false;
  // model/effort 변경으로 세션 재시작이 예약됐을 때 패널 안에만 표시할 플래그
  // (대화창에 append 하지 않는다 — 사용자 결정). 패널을 새로 열 때마다 초기화.
  ...
  if (!process.stdin.isTTY) {
    closed = true;
    resolveDone();
    return { appendDelta() {}, finalizeTurn() {}, appendSystem() {}, appendToolEvent() {}, setLiveThinking() {}, setState() {}, setMood() {}, setSessionMeta() {}, setTurnUsage() {}, setPetVitals() {}, setAgentEmotion() {}, loadHistory() {}, done };
  }
  ...
  const buildChatLines = (width) => {
    ...
    visibleEntries.forEach((e, i) => {
      ...
      // reaching here, so they're untouched. renderChatBody is block-aware
      // (fenced ``` code) on top of the same renderInlineMarkdown+wrapCols
      // pipeline used below it — see its header comment for the no-fence
      // fast path that keeps this byte-identical to the old direct call.
      const body = renderChatBody(bodyText, Math.max(1, width - labelW));
  ...
  const draw = () => {
    ...
    if (compact) {
      ...
      const avatarBody = [
        ...
        '',
        ' ' + stateLine,
        // 사고 과정 실시간 표시 — reasoning 토글(showThinking)이 켜져 있고
        // thinking 중이며 아직 받은 블록이 있을 때만 한 줄 보여준다. OFF면
        // '● thinking' 점만 보이고 이 줄은 아예 나오지 않는다(사용자 결정).
        // 완료/idle로 돌아가면 setState('idle')/finalizeTurn이 liveThinking을
        // 비우므로 다음 draw()에서 자동으로 사라진다.
        ...(state === 'thinking' && showThinking && liveThinking
          ? [' ' + C.muted2 + truncateCols('💭 ' + liveThinking, avatarInner - 2) + C.RESET]
          : []),
        '',
    ...
    if (panelOpen) {
      ...
      overlayCenteredBox(lines, totalWidth, '설정', buildAssistantSettingsPanelBody(
        panelFocus, panelModelIdx, panelEffortIdx, showThinking, panelInnerWidth, settingsPending
      ));
    }
  ...
  const onKey = (str, key) => {
    ...
    // 가장 먼저 처리한다.
    if (str === '\x1c') { panelOpen = !panelOpen; if (panelOpen) settingsPending = false; draw(); return; }
    ...
    if (panelOpen) {
      ...
      // [F/P/R/T] 펫 상호작용 — feed/play/rest(잠)/pet(쓰담). 714af26 에서
      // Ctrl+\ 패널이 설정 오버레이로 재설계되며 빠졌던 다마고치 컨트롤을
      // 같은 패널에 복원한다(사용자 결정: 아바타는 우측에 그대로, 여기는
      // 액션 단축키만). 실제 벡터 연산은 launcher.js 의 onPetInteract 가
      // petLib.interact 로 이미 구현해뒀으므로 여기서는 그걸 호출해 받은
      // 새 vitals 로 이 화면의 petVitals 클로저 변수만 갱신한다 — 펫 로직을
      // ui.js 에 다시 구현하지 않는다.
      const PET_KEY_KIND = { f: 'feed', p: 'play', r: 'rest', t: 'pet' };
      if (Object.prototype.hasOwnProperty.call(PET_KEY_KIND, lower)) {
        const kind = PET_KEY_KIND[lower];
        const nv = (typeof model.onPetInteract === 'function') ? model.onPetInteract(kind) : null;
        if (nv) petVitals = nv;
        draw();
        return;
      }
      ...
      const notifySettingsChange = () => {
        if (typeof model.onSettingsChange !== 'function') { draw(); return; }
        let restarted = false;
        try {
          restarted = model.onSettingsChange({
            model: ASSISTANT_MODEL_OPTIONS[panelModelIdx],
            effort: ASSISTANT_EFFORT_OPTIONS[panelEffortIdx],
            reasoning: showThinking,
          });
        } catch {}
        // model/effort 변경(restart) 시에만 패널 안에 안내를 띄운다. reasoning
        // 토글은 즉시 반영이라 안내 불필요. draw() 로 패널을 다시 그린다.
        if (restarted) settingsPending = true;
        draw();
      };
  ...
  return {
    ...
    finalizeTurn(finalText) {
      ...
      streamingEntry = null;
      mood = 'happy';
      liveThinking = ''; // 턴이 끝나면 우측 패널의 사고 과정 줄도 비운다
      draw();
    },
    appendSystem(text) {
      if (closed) return;
    ...
    // 사고 과정(thinking) 블록 — 더 이상 트랜스크립트에 인라인으로 쌓지
    // 않는다(과거 appendThinking/who==='think' 엔트리). 우측 "Agent" 패널이
    // 그릴 최신 블록 문자열만 갱신한다. 한 턴에 여러 블록이 올 수 있으므로
    // (실시간) 매 호출마다 최신값으로 덮어쓰고 다시 그린다.
    ...
    setState(next) {
      ...
      else if (mood === 'thinking') { mood = 'idle'; liveThinking = ''; }
      draw();
    },
    setMood(next) {
      if (closed) return;
...
module.exports = { setColors, getPalette, setFrameConfig, menu, homeMenu, message, messageInnerWidth, scrollableMessage, liveScrollableMessage, flashMessage, assistantChatView, input, clear, out, W, IW, colWidth, truncateCols, makeBox, clampBoxKeepingBorders, renderHeader, enterAltScreen, exitAltScreen, showCursor, renderInlineMarkdown, renderChatBody, wrapCols };
