...
// Ctrl+글자 → lib/pet 의 INTERACTIONS kind. 힌트 줄(^F밥 ^P놀기 ^R잠 ^T쓰담)과 1:1.
...
function assistantChatView(model) {
    ...
    if (compact) {
      ...
      const spriteSection = skin
        ? [
            ...
            '',
            ' ' + C.muted2 + '^F밥 ^P놀기 ^R잠 ^T쓰담' + C.RESET,
          ]
  ...
  const onKey = (str, key) => {
    ...
    // 상호작용 단축키는 Ctrl+글자만 받는다 — 일반 타이핑은 key.ctrl 이 절대 서지
    // 않으므로 채팅 입력('f'/'p'/'r'/'t' 로 시작하는 메시지 등)과 절대 충돌하지 않는다.
    if (skin && key && key.ctrl && PET_INTERACT_KEYS[key.name]) {
      const kind = PET_INTERACT_KEYS[key.name];
      try {
