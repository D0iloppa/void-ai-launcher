...
// 제목만 바꾸는 얇은 wrapper — applyUpsert가 이미 sessionId로 기존 항목을
// 찾아 title만 갱신하고 startedAt/lastAt(둘 다 null이면)을 보존하므로, 여기선
// title만 담아 upsertConversation을 그대로 재사용한다.
function renameConversation(profileName, sessionId, title) {
  upsertConversation(profileName, { sessionId, title });
}

module.exports = { getConversations, upsertConversation, deleteConversation, renameConversation, applyUpsert, applyDelete, MAX_ENTRIES };
