...
// void-omni-persistent 프로필(experiments 서브메뉴)의 격리 config 디렉토리 —
// void-persistent(위 resolvePersistProfileDir)는 항상 claude 전용 단일 활성
// 슬롯이지만, void-omni-persistent 는 프로필마다 독립된 toolCommand(claude/
// codex/agy)를 가지므로 leaf 에 tool 을 포함시켜 실제 세션(.<tool>-<name>) 및
// void-persistent(.claude-persist-<name>) 와 렉시컬하게 겹치지 않게 한다.
function resolveOmniPersistProfileDir(toolCommand, name) {
  const tool = (toolCommand || '').toLowerCase();
  const leaf = `.omni-persist-${tool}-${name}`;
  return firstWritableDir([
    path.join(os.homedir(), leaf),
    path.join(storageDir(), 'omni-persistent', leaf),
    path.join(os.tmpdir(), 'void-launcher', 'omni-persistent', leaf),
  ]);
}
