...
// void-persistent — phase 1 (수동 계정 전환) ONLY.
//
// 이 모듈이 이 기능의 유일한 로직 보유처다: configDb.js/storage.js 는 얇은
// 저장 접근자만 갖고, xtermFrame.js 는 opts.voidPersistent 로 게이팅된
...
function addPoolMember(name, toolCommand = 'claude') {
  ...
  configDb.setVoidPersistentSwitcher(state);
...
// 독립적으로 재현한다.)
async function runVoidPersistentSession(tool, c, config) {
  ...
  while (true) {
    const currentState = configDb.getVoidPersistentSwitcher();
    ...
    const label = `✳ ${tool.name}  [void-persistent:${member.name}]`;
    ...
    const wrapOpts = {
      ...
      voidPersistent: {
        ...
        // lib/void-persistent/autoSwitchDriver.js)가 남긴 pendingRestart 를
...
module.exports = {
  isAvailable,
  createVoidPersistentProfile,
  eligibleSessions,
  addPoolMember,
  removePoolMember,
  switchTo,
  captureLastSession,
  runVoidPersistentSession,
  encodeCwd,
};
