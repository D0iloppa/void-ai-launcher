...
// 않고 그대로 재사용). 주의: /api/combos 는 admin 관리 키(OMNIROUTE_key)로만
// 인증되므로, 이 함수는 이 머신이 docker/models 체크아웃 + 그 admin 키를 가진
// 경우에만 콤보를 돌려준다 — 진짜 외부(원격) omniroute 인스턴스를 가리키는
...
function listCombos() {
  ...
  if (!parsed || parsed.ok !== true || !Array.isArray(parsed.combos) || parsed.combos.length === 0) {
    return { ok: false, combos: [] };
  }
  return { ok: true, combos: parsed.combos };
}
