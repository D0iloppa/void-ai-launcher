...
//
// 이후 라운드(claude 4-슬롯 모델): claude 프로필은 더 이상 단일 `model` 필드를
// 쓰지 않는다 — Opus/Sonnet/Haiku/Fable 네 슬롯 각각에 실제 모델 id 를 배정해
// (opusModel/sonnetModel/haikuModel/fableModel) ANTHROPIC_DEFAULT_*_MODEL 로
// 얹고 ANTHROPIC_MODEL 은 시작 티어로 'opus' 고정 — 실행 후 Claude Code 자체
// /model 피커로 이름 그대로 opus/sonnet/haiku/fable 전환이 가능하다(자세한
// 내용은 buildLaunchEnv 주석 참고). codex 프로필은 기존 단일 `model` 필드
// 그대로 — 이 변경의 영향을 받지 않는다.
...
//  - claude: 4-슬롯 설계 — 콤보 이름 하나를 ANTHROPIC_MODEL 에 박아 넣으면
//    Claude Code 의 /model 피커에는 "opaque custom model" 하나로만 보여 콤보
//    안의 개별 실제 모델을 고를 수 없다. 대신 Opus/Sonnet/Haiku/Fable 네 슬롯
//    각각에 실제 모델 id 를 하나씩 배정해 ANTHROPIC_DEFAULT_OPUS_MODEL /
//    ANTHROPIC_DEFAULT_SONNET_MODEL / ANTHROPIC_DEFAULT_HAIKU_MODEL /
//    ANTHROPIC_DEFAULT_FABLE_MODEL 로 얹고, ANTHROPIC_MODEL 은 시작 티어로
//    항상 리터럴 'opus' 를 준다(프로필 필드가 아니다) — 실행 후 세션 안에서
//    Claude Code 자체 /model 명령으로 sonnet/haiku/fable 로 자유롭게 전환
//    가능하다(각 이름이 실제로 다른 모델로 해석됨). 로컬에서 4슬롯 전부
//    실측 검증됨(2026-07, http://localhost:20128/v1 대상 `claude -p` 완료
//    응답 확인). ANTHROPIC_BASE_URL/ANTHROPIC_AUTH_TOKEN(AUTH_TOKEN 이지
//    API_KEY 가 아님) + ANTHROPIC_API_KEY 를 강제로 빈 문자열로 비워서 부모
//    셸에서 상속된 진짜 키가 AUTH_TOKEN 을 이기지 못하게 하는 것은 이전과
//    동일하다.
...
function buildLaunchEnv(toolCommand, profile) {
  if (toolCommand === 'claude') {
    return {
      ...
      ANTHROPIC_DEFAULT_FABLE_MODEL: profile.fableModel,
      ANTHROPIC_MODEL: 'opus', // 시작 티어 — 실행 후 Claude Code 자체 /model 로 전환
    };
  }
...
function createProfile({
  name, toolCommand, omniroute_url, omniroute_api_key,
  ...
  if (!omniroute_api_key) {
    return { ok: false, error: 'omniroute_api_key 는 필수입니다.' };
  }

  // toolCommand 별로 모델 관련 필드가 완전히 갈린다 — claude 는 4-슬롯,
  // codex 는 기존 단일 model 필드 그대로.
  if (toolCommand === 'claude') {
    if (!opusModel || !sonnetModel || !haikuModel || !fableModel) {
      return { ok: false, error: 'claude 프로필은 opusModel/sonnetModel/haikuModel/fableModel 이 모두 필요합니다.' };
    }
  } else if (toolCommand === 'codex') {
    if (!model) {
      return { ok: false, error: 'model 은 필수입니다.' };
    }
  }
