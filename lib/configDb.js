...
//   settings:general        → { anonymous_home_prefix, double_width_emoji, wrapper_hpad, wrapper_vpad, windows_use_tmux }
//   experiments:switcher    → { enabled, persistDir, pool:[...], activePoolIndex, autoMode, autoState }
//                              단일 로우, EXPERIMENTAL 계정 스위처(phase 1: 수동 전환만) 설정.
...
// ── EXPERIMENTAL: 계정 스위처 (phase 1: 수동 전환) ──────────────────────────
// getSettings/setSettings 와 동일한 getDb()-null-guard + try/catch-swallow
// 패턴을 그대로 따른다. dJinn 이 불가하면(설치 실패 등) 조용히 기본값/no-op 으로
// degrade 한다 — 이 기능은 실험적 부가 기능이라 부팅을 막을 이유가 없다.
function defaultExperimentSwitcher() {
  return { enabled: false, persistDir: null, pool: [], activePoolIndex: -1, autoMode: false, autoState: {} };
}

function getExperimentSwitcher() {
  const instance = getDb();
  if (!instance) return defaultExperimentSwitcher();
  try {
    const doc = instance.get(COLLECTION, 'experiments:switcher');
    if (doc) return { ...defaultExperimentSwitcher(), ...stripId(doc) };
    return defaultExperimentSwitcher();
  } catch {
    return defaultExperimentSwitcher();
  }
}

function setExperimentSwitcher(obj) {
  const instance = getDb();
  if (!instance) return;
  try {
    const merged = { ...defaultExperimentSwitcher(), ...(obj && typeof obj === 'object' ? obj : {}) };
    instance.put(COLLECTION, 'experiments:switcher', merged);
  } catch {}
}
...
module.exports = {
  ...
  // EXPERIMENTAL 계정 스위처
  getExperimentSwitcher, setExperimentSwitcher,
