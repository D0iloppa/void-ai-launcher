...
function runTmuxSession(toolObj, env, label, opts = {}) {
  ...
  const dbgLog = msg => { try { fs.appendFileSync(path.join(storageDir(), 'tmux-debug.log'), `[${new Date().toISOString()}] ${msg}\n`); } catch {} };
  ...
  dbgLog(`tmux -V status=${tmuxV.status} out=${(tmuxV.stdout||'').trim()} err=${(tmuxV.stderr||'').trim()}`);
  if (tmuxV.status !== 0) { dbgLog('RETURN false: tmux not found'); return false; }
  if (!process.stdin.isTTY || !process.stdout.isTTY) { dbgLog('RETURN false: not TTY'); return false; }
  ...
  if (isWin) {
    ...
    const winPaneCmd = `tmux -L ${sock} wait-for ${chan} & ${winPaneRun}`;
    createPaneArgs = ['cmd', '/c', winPaneCmd];
  } else {
  ...
  dbgLog(`new-session status=${create.status} stderr=${(create.stderr||'').trim()}`);
  ...
  if (create.status !== 0) {
    ...
    dbgLog('RETURN false: new-session failed');
    return false;
  }
  dbgLog('new-session OK — calling attach-session');
  ...
  try {
    ...
    for (const a of cfgs) spawnSync('tmux', ['-L', sock, ...a], { encoding: 'utf8' });
    ...
    let poller;
    if (isWin) {
      const pollerCode = `const {spawnSync}=require('child_process'),sock=${JSON.stringify(sock)},chan=${JSON.stringify(chan)},sa=new Int32Array(new SharedArrayBuffer(4));for(let i=0;i<100;i++){const r=spawnSync('tmux',['-L',sock,'list-clients','-t','m'],{encoding:'utf8'});if(r.status===0&&r.stdout.trim())break;Atomics.wait(sa,0,0,50);}spawnSync('tmux',['-L',sock,'wait-for','-S',chan]);`;
      poller = spawn(process.execPath, ['--eval', pollerCode], { stdio: 'ignore' });
    } else {
      poller = spawn('bash', ['-c',
    ...
    // 풀스크린 attach (블로킹)
    dbgLog('attach-session START');
    const attachResult = spawnSync('tmux', ['-L', sock, 'attach-session', '-t', 'm'], { stdio: 'inherit' });
    dbgLog(`attach-session END status=${attachResult.status}`);
  } finally {
