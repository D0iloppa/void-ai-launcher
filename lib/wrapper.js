...
function findGitBash() {
  ...
  const isWinCodex = isWin && (toolObj.command || '').toLowerCase() === 'codex';
  const gitBash = isWinCodex ? findGitBash() : null;
  ...
  if (isWin) {
    ...
    if (isWinCodex) {
      if (gitBash) {
        createPaneArgs = [gitBash, '-lc', paneCmd];
      } else {
        const winPaneCmd = `tmux -L ${sock} wait-for ${chan} & cmd /d /q /k ${winPaneRun}`;
        createPaneArgs = [process.env.ComSpec || 'cmd', '/d', '/c', winPaneCmd];
      }
    } else {
  ...
  try {
    ...
    let poller;
    if (isWin) {
      const settleMs = isWinCodex ? 1500 : 0;
