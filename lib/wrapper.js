...
function runTmuxSession(toolObj, env, label) {
  const { spawnSync } = require('child_process');
  ...
  const create = spawnSync('tmux', [
    '-L', sock,
    'new-session', '-d', '-s', 'm',
    '-x', String(cols), '-y', String(rows - 1), // -1 for status bar row
    ...extraE,
    'env', '-u', 'TMUX', '-u', 'TMUX_PANE', '-u', 'TMUX_PLUGIN_MANAGER_PATH',
    toolObj.command, ...(toolObj.args || []),
  ], { encoding: 'utf8' });
  ...
  const cfgs = [
    ...
    ['set-option', 'status-style', 'fg=black,bg=colour2,bold'],
    ['set-option', 'status-left', ` ▸ void  ${safeLabel}  `],
    ['set-option', 'status-right', ' ^Space: menu '],
    ['set-option', 'window-status-format', ' #I:#W '],
    ['set-option', 'window-status-current-format', ' [#I:#W] '],
    ['set-option', 'window-status-current-style', 'fg=colour2,bg=black,bold'],
    ['set-option', 'prefix', 'None'],
    ['set-option', 'prefix2', 'None'],
    ['set-option', 'default-terminal', 'xterm-256color'],
    ['bind-key', '-n', 'C-Space', `display-popup -E -w 80% -h 80% '${process.execPath} "${panelPath}" --sock ${sock}'`],
  ];
...
module.exports = { runWrapped, runWrappedShell, runWrappedTmuxFrame, runTmuxWrapped, runTmuxSession };
