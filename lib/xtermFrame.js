...
// experimentSwitch/switchCursor: EXPERIMENTAL phase-1 account-switch seam.
...
// runXtermWrapped's opts.experimentSwitch (see below). Phase 2 (auto-mode)
// would extend experimentSwitch with its own fields here — not started.
function composite(term, cols, rows, hpad, vpad, label, helpText, panelOpen = false, panelSubView = null, subViewData = null, modeHint = null, experimentSwitch = null, switchCursor = null, hasMessaging = false, mailListCursor = null) {
  const { topM, botM, leftM, rightM } = computeMargins(cols, rows, hpad, vpad);
  ...
  if (panelOpen) {
    ...
    if (panelSubView && Array.isArray(subViewData)) {
      ...
      if (boxW >= 10 && boxH >= 3) {
        ...
        const MAIL_TITLES = {
          ...
          'mail-send': ' 받는 사람 선택 ', 'mail-send-compose': ' 메시지 작성 ',
          'mail-accept-confirm': ' 메시지 수락 ',
        };
        ...
        for (let i = 0; i < boxH - 2; i++) {
          ...
          // param from switchCursor so the experimental switch subview's own
          // cursor state is never touched by this addition.
          const isCursorRow = (panelSubView === 'switch' && switchCursor === i)
            || (['mail', 'mail-inbox', 'mail-send'].includes(panelSubView) && mailListCursor === i);
    ...
    const panelLabel = panelSubView === 'switch'
      ...
      // experimentSwitch (EXPERIMENTAL, phase 1): appends the S-key hint only
      ...
      // lib/messaging loaded successfully — no experiment flag needed.
      : ' Control Panel  ·  H: 도움말  ·  U: 사용량  ·  Enter: 홈으로 복귀  ·  Ctrl+\\ / Esc: 패널 닫기 '
        + (experimentSwitch ? '  ·  S: 계정 전환 ' : '')
        + (experimentSwitch ? `  ·  X: 자동 모드 [${experimentSwitch.autoMode ? 'ON' : 'OFF'}] ` : '')
        + (hasMessaging ? '  ·  M: 메시징 ' : '');
...
// ── Mailbox "accept" prompt builder (Phase B) ───────────
// Pure and side-effect-free so it's testable without a live PTY: given a
// message's absolute .md path, returns the exact text the mail-accept-confirm
// flow (see runXtermWrapped's 'mail-accept-confirm' subview below) injects
// into the wrapped child's input.
...
async function runXtermWrapped(tool, env, label, opts = {}) {
  ...
  const path = require('path'); // used by the [a] accept flow to resolve a mail message's absolute path
  let pty;
  ...
  // ── EXPERIMENTAL account switcher (phase 1 manual + phase 2 auto) ────────
  // opts.experimentSwitch is undefined for every existing caller (runner.js's
  ...
  // lib/experiments/switchProfile.js's runExperimentSession opts in.
  const experimentSwitch = opts.experimentSwitch || null;
  ...
  let switchCursor = experimentSwitch && typeof experimentSwitch.activePoolIndex === 'number' && experimentSwitch.activePoolIndex >= 0
    ? experimentSwitch.activePoolIndex : 0;
  const buildSwitchOverlay = () => {
    if (!experimentSwitch || !Array.isArray(experimentSwitch.pool)) return ['전환 가능한 계정이 없습니다'];
    if (experimentSwitch.pool.length === 0) return ['pool 이 비어 있습니다'];
    return experimentSwitch.pool.map((m, i) => {
      const active = i === experimentSwitch.activePoolIndex ? ' (활성)' : '';
      return `${m.name}  [${m.toolCommand || 'claude'}]${active}`;
    });
  };
  // Phase 2 (auto-mode): the 'X' key toggle lives further down alongside the
  // 'S' key handler; opts.experimentSwitch.pollPendingRestart is polled from
  // barTimer below — see both call sites for the full seam.
  const pollPendingRestart = experimentSwitch && typeof experimentSwitch.pollPendingRestart === 'function'
    ? experimentSwitch.pollPendingRestart : null;
  ...
  let mailListCursor = 0;      // cursor within 'mail-inbox' / 'mail-send' lists
  ...
  // [a] accept (Phase B): double-confirm state for injecting a message's path
  // into the wrapped child's input. mailAcceptReturnView remembers which mail
  // subview to fall back to on Esc ('mail-inbox' or 'mail-detail').
  let mailAcceptFile = null;
  ...
  const formatMailRow = entry => {
    ...
    return `${box} ${dot} ${entry.preview || '(내용 없음)'}  ${ts}`;
  };
  ...
  // [a] accept (Phase B) — double-confirm popup shown before injecting a
  // message's absolute .md path into the wrapped child's input. promptText is
  // the exact string that will be written verbatim (no trailing newline) via
  // child.write() on Enter.
  const buildMailAcceptConfirmOverlay = promptText => [
    promptText,
    '',
    `'${promptText}' 이 메시지를 입력창에 기재합니다.  [enter: 승락]  [esc: 취소]`,
  ];
  ...
  function paint() {
    if (done) return;
    // Mail subviews share one highlighted-row concept across three different
    // local cursor variables ('mail' uses mailCursor, 'mail-inbox'/'mail-send'
    // use mailListCursor) — composite() just needs whichever one is live.
    const mailCursorForPaint = panelSubView === 'mail' ? mailCursor
      : (panelSubView === 'mail-inbox' || panelSubView === 'mail-send') ? mailListCursor
      : null;
    process.stdout.write(composite(term, cols, rows, hpad, vpad, label, helpText, panelOpen, panelSubView, subViewData, currentGaugeText(), experimentSwitch, switchCursor, Boolean(messaging), mailCursorForPaint));
  }
  ...
  const togglePanel = () => {
    ...
    if (!panelOpen) {
      ...
      mailAcceptFile = null; mailAcceptPromptText = null; mailAcceptReturnView = null;
    }
    const d = currentDims();
  ...
  const onStdin = data => {
    ...
    if (panelOpen) {
      // 'switch' sub-overlay (EXPERIMENTAL, phase 1): unlike help/usage it is
      ...
      if (panelSubView === 'switch') {
        if (input.includes('\x1b[A') || input === 'k') {
          if (experimentSwitch && Array.isArray(experimentSwitch.pool) && experimentSwitch.pool.length > 0) {
            switchCursor = (switchCursor - 1 + experimentSwitch.pool.length) % experimentSwitch.pool.length;
            scheduleRepaint();
          }
          return;
        }
        if (input.includes('\x1b[B') || input === 'j') {
          if (experimentSwitch && Array.isArray(experimentSwitch.pool) && experimentSwitch.pool.length > 0) {
            switchCursor = (switchCursor + 1) % experimentSwitch.pool.length;
            scheduleRepaint();
          }
          return;
        }
      ...
      if (panelSubView === 'mail') {
        ...
        if (input.includes('\x1b[A') || input === 'k' || input.includes('\x1b[B') || input === 'j') {
      ...
      if (panelSubView === 'mail-inbox') {
        ...
        // [a] accept — PHASE B. Cursor row's message: build the injection
        // prompt and drop into the double-confirm popup before touching the
        // child's input at all.
        if (input === 'a' || input === 'A') {
          const entry = entries[mailListCursor];
          if (entry && messaging) {
            mailAcceptFile = entry.file;
            mailAcceptPromptText = buildMailAcceptPrompt(path.resolve(entry.file));
            mailAcceptReturnView = 'mail-inbox';
            panelSubView = 'mail-accept-confirm';
      ...
      if (panelSubView === 'mail-detail') {
        ...
        // [a] accept — PHASE B. Currently-open message: same double-confirm
        // flow as the 'mail-inbox' branch above.
        if (input === 'a' || input === 'A') {
          if (mailDetailFile && messaging) {
            mailAcceptFile = mailDetailFile;
            mailAcceptPromptText = buildMailAcceptPrompt(path.resolve(mailDetailFile));
            mailAcceptReturnView = 'mail-detail';
            panelSubView = 'mail-accept-confirm';
            subViewData = buildMailAcceptConfirmOverlay(mailAcceptPromptText);
            scheduleRepaint();
          }
      ...
      // [a] accept — PHASE B double-confirm popup. Enter injects
      // mailAcceptPromptText verbatim (no trailing newline — the user presses
      // Enter themselves once it's in the child's input line) via the SAME
      // child.write() call site used for normal keystrokes below, then closes
      // the panel via the existing togglePanel() close path so focus returns
      // to the child. Esc cancels back to whichever mail subview opened it,
      // without writing anything to the child.
      if (panelSubView === 'mail-accept-confirm') {
        if (/^[\r\n]+$/.test(input)) {
          if (mailAcceptPromptText) {
            try { child.write(mailAcceptPromptText); } catch {}
            if (messaging && mailAcceptFile) {
              try { messaging.mailbox.markReadOne(mailAcceptFile); } catch {}
            }
          }
          mailAcceptFile = null;
      ...
      if (panelSubView === 'mail-send') {
        ...
        if (input.includes('\x1b[A') || input === 'k') { mailListCursor = (mailListCursor - 1 + count) % count; scheduleRepaint(); return; }
        if (input.includes('\x1b[B') || input === 'j') { mailListCursor = (mailListCursor + 1) % count; scheduleRepaint(); return; }
        if (/^[\r\n]+$/.test(input)) {
      ...
      if (panelSubView === 'mail-send-compose') {
        ...
        if (/^[\r\n]+$/.test(input)) {
          if (messaging && mailSendTarget && mailComposeBuf.trim().length > 0) {
            try {
              if (mailSendTarget === '*') messaging.mailbox.broadcast(mailComposeBuf, { registry: messaging.registry });
              else messaging.mailbox.sendTo(mailSendTarget, mailComposeBuf, { registry: messaging.registry });
      ...
      // 'S' (EXPERIMENTAL, phase 1) — only reachable when the caller opted in
      // via opts.experimentSwitch; otherwise falls through unmatched, same as
      // any other unbound key already does.
      if (experimentSwitch && /^[sS]+$/.test(input)) {
        panelSubView = 'switch';
        subViewData = buildSwitchOverlay();
        scheduleRepaint();
        return;
      }
      // 'X' (EXPERIMENTAL, phase 2) — toggles experiments:switcher.autoMode.
      // Only reachable when the caller opted in via opts.experimentSwitch;
      // otherwise falls through unmatched, same as the S-key above.
      if (experimentSwitch && /^[xX]+$/.test(input)) {
        try {
          const configDb = require('./configDb');
          const current = configDb.getExperimentSwitcher();
          current.autoMode = !current.autoMode;
          configDb.setExperimentSwitcher(current);
          experimentSwitch.autoMode = current.autoMode;
        } catch {}
        scheduleRepaint();
        return;
      }
  ...
  // background auto-switch (set by lib/experiments/autoSwitchDriver.js from
  ...
  // runExperimentSession relaunch with the new creds + --resume. Guarded by
...
function getInputProfile(toolCommand) {
  switch (toolCommand) {
    ...
    case 'agy':
      return { name: 'agy', wrapperMouse: false, mouseEnable: '' };
    default:
...
module.exports = { runXtermWrapped, composite, cellSgr, ptyDims, applyTheme, getInputProfile, buildMailAcceptPrompt };
