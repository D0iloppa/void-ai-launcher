...
async function runXtermWrappedOnce(tool, env, label, opts = {}) {
  ...
  // Mail 'resume' accept 즉시-재시작 seam. opts.experimentSwitch/onControlAction/
            ...
            try {
              if (entry.seedType === 'msg') {
                ...
                if (!landing || !landing.configDir) {
                  ...
                  // SESSION'S OWN configDir (not the experiment pool's shared
...
// menu". Any other exit (normal quit, experiment-switch S-key, auto-mode
// restart) leaves .requested null, so the loop returns the boolean exactly
// like the old single-shot function did.
//
// This is INDEPENDENT of opts.experimentSwitch/onControlAction/opts.restartSignal
// — lib/experiments/switchProfile.js owns its own external while(true) loop
