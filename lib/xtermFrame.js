...
async function runXtermWrapped(tool, env, label, opts = {}) {
  ...
  // 미러링한다 — messaging과 experiments는 서로 다른 관심사라 결합하지 않는다
  ...
  const onStdin = data => {
    ...
    if (panelOpen) {
      ...
      if (panelSubView === 'mail-accept-session') {
        ...
        if (input.includes('\x1b[A') || input === 'k') { if (sessions.length) mailListCursor = (mailListCursor - 1 + sessions.length) % sessions.length; scheduleRepaint(); return; }
        if (input.includes('\x1b[B') || input === 'j') { if (sessions.length) mailListCursor = (mailListCursor + 1) % sessions.length; scheduleRepaint(); return; }
        if (/^[\r\n]+$/.test(input)) {
      ...
      //                   is DEFERRED (see comment below): lands as "run it
      //                   from the menu" instead of relaunching in place.
            ...
            try {
              if (entry.seedType === 'msg') {
                ...
                if (!landing || !landing.configDir) {
                  ...
                  // (same uuid), acks the message, and locks the source
                  // session. Its returned directive {configDir, resumeSessionId,
                  // cwd} is intentionally NOT consumed here.
                  //
                  // Immediate-restart seam — DEFERRED. opts.experimentSwitch's
                  // restartSignal is tightly coupled to the experiment pool's
                  // shared persistDir slot (runExperimentSession always
                  // launches with currentState.persistDir + member.lastSessionId,
                  // never an arbitrary named session's own configDir — see
                  // lib/experiments/switchProfile.js). directive.configDir here
                  // is the LANDING SESSION'S OWN configDir, a different
                  // directory entirely; reusing the seam correctly would need
                  // switchProfile.js changes, which is out of scope for this
                  // pass. Land it instead — the jsonl is already copied, the
                  // user resumes it from the normal menu.
                  messaging.resumeFork.acceptSeed(entry, {
