...
const fs   = require('fs');
const { resolveSessionConfigDir } = require('./storage');
...
async function terminalSessionsMenu(config, c) {
  ...
  while (true) {
    ...
    if (sel.key === '1') {
      ...
      const shellName = shellCmd.endsWith('zsh') ? 'zsh' : shellCmd.endsWith('bash') ? 'bash' : 'sh';
      const toolItems = [
        { key: '1', label: `Host Shell (${shellName})`, desc: shellCmd },
        ...tools.map((t, i) => ({ key: String(i + 2), label: t.name, desc: t.command })),
      ];
      const toolSel = await menu('새 세션 — 도구 선택', toolItems, { back: true });
      if (!toolSel) continue;
      if (toolSel.key === '1') {
        await createSession({ name: `Shell (${shellName})`, command: shellCmd, args: [] }, c);
      } else {
        await createSession(tools[Number(toolSel.key) - 2], c);
      }
    } else {
...
function sessionDirForTool(toolCommand, name) {
  return resolveSessionConfigDir(toolCommand, name);
}

function sessionCapableTools(config) {
  return (config.tools || []).filter(tool => ['claude', 'codex', 'agy'].includes((tool.command || '').toLowerCase()));
}
...
async function cliSessionsMenu(config, c) {
  ...
  while (true) {
    ...
    const sel = await menu('CLI 세션 관리', items, { back: true, enableDelete: true });
    if (!sel) return;

    if (sel.action === 'delete') {
      const s = sessions[Number(sel.key) - 1];
      if (s) {
        const confirmSel = await menu(`세션 삭제: ${s.name}`, [
          { key: '1', label: '세션 등록 해제', desc: 'storage에서 제거 (디렉토리 유지)' },
          { key: '2', label: '완전 삭제', desc: '등록 해제 + 디렉토리 삭제' }
        ], { back: true });
        if (confirmSel) {
          deleteSession(s.name, s.toolCommand || 'claude');
          if (confirmSel.key === '2' && fs.existsSync(s.configDir)) {
            try { fs.rmSync(s.configDir, { recursive: true, force: true }); } catch {}
          }
          await message(c.signal + '삭제 완료' + c.RESET);
        }
      }
      continue;
    }
...
async function createCliSession(config, c) {
  ...
  // (취소하면 아래 `if (!raw) return;`로 자연스럽게 빠져나감, 동작은 그대로).
...
// 토큰 연결은 완전히 선택 사항인 부가 기능이다 — 연결하지 않은 세션은 기존과
// 동일하게 실제 대화형 로그인으로 발급된 .credentials.json 에만 의존해 동작한다
// (lib/runner.js의 applySessionEnv 참고). 여기서는 링크의 등록/해제만 다룬다.
async function sessionDetailMenu(session, c) {
  const { menu, message } = require('./ui');
  const { deleteSession, saveSession } = require('./storage');
  ...
  while (true) {
    ...
    const items = [
      ...
      {
        key: '3', label: '토큰 연결',
        desc: session.tokenService ? `현재: ${session.tokenService}/${session.tokenAlias}` : '연결 안 됨 (선택사항)',
      },
    ];
    if (session.tokenService) {
      items.push({ key: '4', label: '토큰 연결 해제', desc: '이 세션에서 연결된 토큰 제거' });
    }

    const sel = await menu(`세션: ${session.name}`, items, { back: true });
    if (!sel) return;

    if (sel.key === '1' || sel.key === '2') {
    ...
    if (sel.key === '3') {
      ...
      if (picked) {
        session.tokenService = picked.service;
        session.tokenAlias = picked.alias;
        saveSession(session);
        await message(c.ok + `✓ 토큰 연결됨  ${picked.service}/${picked.alias}` + c.RESET);
      }
      continue;
    }

    if (sel.key === '4') {
      delete session.tokenService;
      delete session.tokenAlias;
      saveSession(session);
      await message('토큰 연결이 해제되었습니다.');
      continue;
    }
  }
}
