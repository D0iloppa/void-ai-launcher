...
const { runWrapped, runWrappedShell, runWrappedTmuxFrame, runTmuxSession } = require('./wrapper');
...
async function runTool(tool, mode, c, config, extraArgs = []) {
  ...
  // tmux fullscreen → node-pty frame → plain 순으로 시도
  let wrapped = runTmuxSession(toolObj, env, label);
  if (!wrapped) wrapped = await runWrappedTmuxFrame(toolObj, env, label, wrapOpts);
