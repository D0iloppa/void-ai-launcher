...
async function runTool(tool, mode, c, config, extraArgs = []) {
  ...
  // Windows-only: tmux cannot run natively (no ConPTY backend), so
  // runTmuxSession returns false there. Frame every tool — including codex —
  // via the @xterm/headless compositor instead. Never executes off-Windows.
  if (!wrapped && process.platform === 'win32') {
