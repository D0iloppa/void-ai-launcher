...
const DEFAULT_OMNIROUTE_REFRESH_INTERVAL_SEC = 60;
const OMNIROUTE_SPAWN_TIMEOUT_MS = 8000;

// Self-throttle state for runOmnirouteUsageRefreshIfDue() below — deliberately a plain
// module-scope var (not persisted): a process restart resetting this to "due immediately" is
// fine/expected, mirroring voidNotify's own due-item checks being purely time-based.
...
/*
 * Piggybacked onto the same poll tick as voidNotify's runWorkerOnce() (see the default
 * workerFn in runLoop() below), but self-throttled on its OWN interval
 * (configDb settings' omniroute_usage_refresh_interval_sec, default 60s) rather than
 * VOID_NOTIFY_POLL_MS — the two are independent knobs, same philosophy as voidNotify's
 * due-item check (only act once "due", otherwise no-op).
 *
 * Fail-open throughout, mirroring lib/void-persistent/omnirouteUsageTier.js's own contract:
 * configDb unreadable, router script missing, spawn error/timeout/non-zero exit are all
 * silently swallowed — a failed usage refresh must never affect voidNotify's delivery loop.
 */
function runOmnirouteUsageRefreshIfDue({ now = Date.now, spawnSyncFn = require('child_process').spawnSync } = {}) {
  let intervalSec = DEFAULT_OMNIROUTE_REFRESH_INTERVAL_SEC;
  try {
    const settings = require('./configDb').getSettings() || {};
    intervalSec = typeof settings.omniroute_usage_refresh_interval_sec === 'number'
      ? settings.omniroute_usage_refresh_interval_sec
      : DEFAULT_OMNIROUTE_REFRESH_INTERVAL_SEC;
  } catch {
    // configDb unavailable — fall back to the default interval above
  }

  const nowTs = now();
  if (nowTs - lastUsageRefreshAt < intervalSec * 1000) return; // not due yet

  lastUsageRefreshAt = nowTs;

  try {
    const rp = require('./void-persistent/omnirouteUsageTier').routerPath();
    if (!rp) return; // router script not installed/configured — nothing to do
    spawnSyncFn('python3', [rp, 'usage_refresh'], { timeout: OMNIROUTE_SPAWN_TIMEOUT_MS });
  } catch {
    // fail-open — a failed refresh is a no-op, never surfaced to the caller
  }
}
...
async function runLoop({
  ...
  const worker = workerFn || (async () => {
    await require('./voidNotify').runWorkerOnce();
    try { runOmnirouteUsageRefreshIfDue(); } catch {}
  });
