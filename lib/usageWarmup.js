...
async function warmUsageCache(config) {
  try {
    ...
    // EXPERIMENTAL phase 2 (자동 계정 전환) — 사이클당 1회, TTY/PTY 없이 로컬
    // 로그만 스캔한다. experiments:switcher.autoMode 가 꺼져 있으면 즉시 no-op.
    // 절대 throw 하지 않는다(내부에서 이미 방어적으로 감쌈) — 방어적으로 여기서도 한 번 더 감싼다.
    try { require('./experiments/autoSwitchDriver').checkAutoSwitch(config); } catch {}
