...
function getWarmupTargets() {
  ...
  // 이름 없는(전역) 계정 — claude/codex/agy 각각의 기본 설정 경로.
  const targets = [
    { toolCommand: 'claude', configDir: null, sessionKey: 'default' },
    { toolCommand: 'codex', configDir: null, sessionKey: 'default' },
    { toolCommand: 'agy', configDir: null, sessionKey: 'default' },
  ];

  for (const s of getSessions()) {
    const toolCommand = (s && s.toolCommand) || 'claude';
    // 사용량 조회 메커니즘이 없는 도구는 건너뛴다 (claude/codex/agy 만 지원).
    if (toolCommand !== 'claude' && toolCommand !== 'codex' && toolCommand !== 'agy') continue;
  ...
  try {
    ...
    for (const target of targets) {
      try {
        ...
        } else if (target.toolCommand === 'agy') {
          await getAgyUsage(config, overrides);
        }
      } catch {
