...
| `voidNotify.js` | void-notify Transactional Outbox — `notify({when,subject,content,provider_key})` enqueues a message; `runWorkerOnce()` (driver-agnostic: called once by `void notify-worker`, and repeatedly by `voidDaemon.js`'s resident poll loop) claims due queue items via an atomic lease-CAS transaction (`claimOne`/`claimDueBatch` — real `better-sqlite3` transaction, race-safe across processes) and delivers them through a `kind`→sender map (`telegram` only for now, via `sendTelegram`). Channel registry (level-2 nodes) + queue (level-3 docs, parented under their channel) live in their own dJinn graph, mirroring `voidContext.js`'s structure (`_schema` reserved node, `_isReserved` filtering). Channel `api_key` is stored only as `enc_api_key` (AES-256-GCM via `voidNotifyCrypto.js`) — never plaintext, never returned by any accessor/MCP response |
| `voidNotifyCrypto.js` | AES-256-GCM primitives for void-notify channel secrets — mirrors `lib/sync.js`'s frame crypto (`randomBytes(12)` IV, `createCipheriv('aes-256-gcm', ...)`, `getAuthTag()`) but with a **persistent** key (`storageDir()/void-notify.key`, generated once + cached in-process) instead of sync.js's per-session HKDF key, since void-notify secrets must survive process restarts. `encryptSecret`/`decryptSecret` are pure and unit-testable |
| `voidNotifyMcp.js` | Thin stdio MCP entry point (`node lib/voidNotifyMcp.js`). Unlike `voidContextMcp.js`, does **not** use dJinn's auto-generated `createMcpServer` — that would expose raw `graph_doc_put_void_notify`, letting a client overwrite lease/status fields and break the CAS claim. Hand-rolls a plain `McpServer` (mirrors `voidDispatchMcp.js`) exposing only `notify`/`notify_put_channel`/`notify_list_channels`/`notify_del_channel`/`notify_list_queue` — all channel-listing responses redact `enc_api_key` |
| `voidDaemon.js` | voidemon — the resident singleton daemon that IS the sole scheduler for void-notify (no cron code exists anywhere in this repo; see the void-notify paragraph below). Core loop (`runLoop({pollMs,once,shouldStop,workerFn,sleepFn})`) repeatedly calls `voidNotify.runWorkerOnce()` then sleeps `pollMs` (default 60000ms, override via env `VOID_NOTIFY_POLL_MS`); every iteration is wrapped in try/catch so a throwing poll never kills the loop (fail-open, mirroring voidContext's hook philosophy). Singleton enforcement is a pidfile at `storageDir()/voidemon.pid`: `acquireSingleton` creates it with `fs.openSync(path,'wx')` (atomic O_CREAT\|O_EXCL, works on Windows too — the reason this daemon exists is cron-less OSes), and on `EEXIST` probes the stored pid's liveness via `process.kill(pid,0)` (ESRCH ⇒ stale ⇒ reclaim; alive ⇒ refuse). `releaseSingleton` only deletes the pidfile if it still names the caller's own pid. All fs/kill/pid inputs are injectable seams (mirrors `lib/sync.js`) so the singleton logic is unit-testable without real processes. Driven from `launcher.js`'s `case 'daemon':` (`void daemon start\|stop\|status\|restart`, plus internal `--run` which is the detached child's entry point and the only place that actually calls `acquireSingleton`) |
...
**void-notify**: a separate dJinn DB, `void-notify.djinn.db`, plus its crypto key `void-notify.key` — both under `storageDir()` (`lib/storage.js`, i.e. `~/.config/void-launcher/`), **not** the repo root. This is a deliberate deviation from void-context (which is repo-root/per-checkout by design): void-notify is a per-user resident background service (its queue/channels/key must outlive a repo re-clone or a global `npm install`), whereas void-context is deliberately tied to the working tree it was created in. Do not "fix" this by moving void-notify's DB to the repo root — that would delete a user's pending notifications and encryption key on every re-clone. Namespace `void_notify` (underscore, same `NS_RE` constraint). Initialized by `postinstall` (`scripts/init-void-notify.js`) and exposed via `.mcp.json` (`node lib/voidNotifyMcp.js`). CLI: `void notify-worker` (one-shot poll+deliver — a manual/scripted driver for ad-hoc runs) and `void notify --add-channel/--list-channels/--del-channel/--send/--list-queue` (admin). The resident singleton daemon `void daemon start|stop|status|restart` (`lib/voidDaemon.js`) is now THE scheduler that calls `notify-worker`'s underlying `runWorkerOnce()` on an interval — cron was intentionally not used (no cron install/uninstall code exists in this repo), since the daemon works uniformly across OSes including cron-less Windows. The daemon is not auto-started by `postinstall` or launcher startup; the user runs `void daemon start` explicitly.
